    ...
    <div>
      <div className="max-w-6xl mx-auto mt-4">
        <div className="flex items-center space-x-4 mb-2">
          <div className='relative rounded-lg border-transparent dark:border-[#5f5f5f] dark:hover:border-gray-500 dark:bg-[#424242] flex flex-col items-center border-2 '>
          ...
          <div className='relative rounded-lg w-[170px] flex flex-col items-center my-2'>
            <button onClick={() => setIsOpen(prev => !prev)} className='flex dark:border-[#5f5f5f] dark:hover:border-gray-500 dark:bg-[#424242] items-center w-full bg-zinc-100 p-0 pl-2 justify-between text-sm rounded-lg border-2 border-transparent  duration-300'>
              {filter || 'All Categories'}
              {isOpen ? <IoIosArrowUp className='text-black h-8 pr-1' /> : <IoIosArrowDown className='text-black h-8 pr-1' />}
            </button>
            {isOpen && (
              <div className='w-full bg-zinc-100/75 absolute top-12 left-0 rounded-lg border-2 border-transparent  p-2'>
        ...
        {blogs.map((post) => (
          <div key={post.id} className="mb-10 p-6 dark:hover:border-gray-500 shadow-lg rounded-lg border-transparent dark:border-[#5f5f5f] dark:bg-[#424242] bg-white cursor-pointer border-2 " onClick={() => router.push(`/blogs/${post.id}`)}>
